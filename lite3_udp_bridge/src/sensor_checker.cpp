...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        sensor_checker.imu.check();
        sensor_checker.odom.check();
        sensor_checker.odom2.check();
        sensor_checker.joint.check();
        sensor_checker.realsense.check();
        sensor_checker.lidar.check();
        sensor_checker.ultrasound.check();
        ros::spinOnce();
        loop_rate.sleep();
    }
    return 0;
